...
main(void)
{
        ...
        *shmem_addr = 1;
        ...
        printf(1,"      proc1 called shm_get('test1')\n");
        char *init = "sh";
        char *fs = "/temp";
        int nproc = 6;
        strcpy(shmem_addr,init);
        printf(1,"      proc1 put 'sh' at start of shared page\n");
        shmem_addr += strlen(init) + sizeof(char);
        strcpy(shmem_addr,fs);
        printf(1,"      proc1 put '/temp' after 'sh' in shared page\n");
        shmem_addr += strlen(fs) + sizeof(char);
        *shmem_addr = nproc;
        printf(1,"      proc1 put '6' after '/temp' in shared page\n");
